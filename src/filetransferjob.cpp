...
FileTransferJob::FileTransferJob(const QSharedPointer<QIODevice>& origin, int size, const QUrl& destination, QObject *parent) : QObject(parent)
{
...
void FileTransferJob::sourceFinished()
{
    ...
    if (mSize > -1 && mWritten != mSize) {
        qDebug() << "Received incomplete file (" << mWritten << " of " << mSize << " bytes)";
        //setError(1);
